...
# moving grafana-instance to before the wile loop
oc apply -f 02-grafana-instance.yaml


# while loop never finds an grafan resource even after grafana pod is running in kepler n/s
...
oc apply -f 03-grafana-datasource-UPDATETHIS.yaml
